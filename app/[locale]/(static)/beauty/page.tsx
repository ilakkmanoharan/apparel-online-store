...
export default async function BeautyPage({ params }: BeautyPageProps) {
  const products = await getProductsByCategory("beauty", params.locale);
  const { t } = await getServerTranslator(params.locale);

  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold mb-4">{t("static.beauty.title")}</h1>
      <p className="text-gray-600 mb-6">{t("static.beauty.description")}</p>
      {products.length === 0 ? (
